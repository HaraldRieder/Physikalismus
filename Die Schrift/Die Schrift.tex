...
\ref{eq:probabilities} wird in der Quantenmechanik nun in dieser Art geschrieben:
\begin{equation} \label{eq:probabilities_qm}
\ket{\psi_0} \xrightarrow{\textrm{Messung 1}} 
\ket{\psi_1} \xrightarrow{\textrm{Messung 2}}  \textrm{usw.}
\end{equation}

Jeder Zustandsvektor $\psi_i$ enthält alle Wahrscheinlichkeiten für alle möglichen Ereignisse. Über die möglichen Ereignisse müssen wir uns noch unterhalten...

Ein Kenner der nichtrelativistischen Quantenmechanik mag nun vielleicht einwerfen, dass sich die Zustandsvektoren $\psi_i$ (oder Operatoren im Heisenbergbild) und damit die Wahrscheinlichkeitsmengen mit der Zeit von selbst kontinuierlich verändern zwischen den einzelnen sprunghaften Ereignissen. Anhänger der Viele-Welten-Interpretation, die in Wahrheit eine unvollständige Viele-Welten-Theorie ist, können gar der Auffassung sein, dass es \emph{ausschließlich} eine objektive, vom Bewusstsein unabhängige, kontinuierliche Veränderung in der objektiv vergehenden physikalischen Zeit, die sogenannte \emph{Schrödinger-Dynamik}, gäbe. In einem späteren Abschnitt werden wir sehr gute Gründe kennenlernen, \emph{warum im physikalischen Kanal keine Zeit von selbst vergeht}, und warum es sehr kompliziert wird, wenn man an eine objektive Schrödinger-Dynamik glaubt.
