...
\end{equation}
mit mehr als einem Summand steht für eine Verschränkung der Teilräume $\mathscr{H}$ und $\mathscr{E}$. Die Darstellung (\ref{eq:general_solution}) von $\ket{\psi}$ ist eine \href{https://en.wikipedia.org/wiki/Schmidt_decomposition}{Schmidt-Zerlegung}.
...
\item Was genau sich in \eqref{eq:conditional_probability} zeitlich entwickelt, ist ontologisch nicht klar. So lange die Wahrscheinlichkeit sich dabei nicht ändert, kann die Zeitentwicklung beliebig zwischen Operator und Vektoren hin- und hergeschoben werden, was in den verschiedenen "Bildern", also Schrödinger-, Heisenberg- und Wechselwirkungsbild, ausgenutzt wird.
\item $\psi^{(0)}$ und $\psi^{(1)}$ sind Vektoren aus dem $\mathscr{H}$ Unterraum. Auch in der relativistischen Quantenmechanik wird das \href{http://www.itp.uni-bremen.de/~noack/dirac.pdf}{invariante Skalarprodukt} definiert als \emph{drei}dimensionales Integral proportional zu 
$\int \frac{\mathrm{d}^3 \vec{k}}{k_0}\ \psi^*(k)\phi(k)$. Diese Integration "auf der Massenschale" entspricht der Vermeidung der Zustände (\ref{eq:general_state}) mit $j \neq k$.
\item Es handelt sich um die ursprüngliche Formulierung (\ref{eq:orig_Born}) der Bornschen Wahrscheinlichkeit. Der Verschränkungsprozess mit der Umgebung ist nicht ausmodelliert.
\end{itemize}
