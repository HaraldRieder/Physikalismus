...
\ \xrightarrow{p^{(0)\rightarrow(k)}\ =\ \left|a_k\right|^2}\ 
...
\begin{equation*}
\ket{t_k} \otimes\ket{\psi^{(k)}}\ =\ 
\hat{u}^k \ket{t_0} \otimes \ket{\psi^{(0)}}
\end{equation*}
Dabei haben wir $t_0 = 0$ gesetzt, was wir nach jedem Prozessschritt neu tun können, da die Wahrscheinlichkeiten nur von der Zeitdifferenz abhängig sind.

Wir haben für den "Kollaps" mehr Zielzustände als in \eqref{eq:conditional_probability}, eben alle aus $\mathscr{H}\otimes\mathscr{E}$ im Gegensatz zu allen aus $\mathscr{H}$. Dadurch sind die Amplituden selbstverständlich anders zu normieren. 

Es ist vollkommen unbestimmt, mit welcher Wahrscheinlichkeit eine bestimmte Zeit $t_k$ beobachtet wird. Für jede bestimmte Zeit $t_k$ müssen die Wahrscheinlichkeiten aber wie in \eqref{eq:conditional_probability} gewichtet sein. 

\begin{equation*}
\frac{\left|\bra{\lambda_k} \hat{u}^k \ket{\psi^{(0)}}\right|^2}{\sum }
\end{equation*}
