...
Weil wir es noch benötigen werden, kümmern wir uns etwas, das du in dieser Form anderweitig schwer finden dürftest. Eine Funktion $\psi$ ordnet jedem Element $\displaystyle x$ einer Definitionsmenge $\displaystyle D$ genau ein Element $\displaystyle y$ einer Zielmenge $\displaystyle Z$ zu. 

\begin{equation*}
f \colon \,{\begin{cases}D\to Z\\ x \mapsto y(x)\end{cases}}
\end{equation*}

Eine \emph{Umindexierung} wollen wir in dieser Schrift eine \emph{raumfüllende Kurve} basierend auf einer \emph{umkehrbaren} Abbildung
...
\end{equation*}
nennen, die ein zusammenhängendes Interval in $\mathbb{R}$ auf ein zusammenhängendes Gebiet in $\mathbb{R}^N$ abbildet.

Das Kurvenintegral einer stetigen (skalaren) Funktion $\psi$ ist definiert als
\begin{equation*}
\int \limits _{\mathcal {C}}\psi\,\mathrm{d} s := 
