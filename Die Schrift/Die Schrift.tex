...
\subsection{Umindexierung}

Weil wir es noch benötigen werden, kümmern wir uns etwas, das du in dieser Form anderweitig schwer finden dürftest. Eine Funktion $f$ ordnet jedem Element $\displaystyle x$ einer Definitionsmenge $\displaystyle D$ genau ein Element $\displaystyle y$ einer Zielmenge $\displaystyle Z$ zu. 

\begin{equation*}
f \colon \,{\begin{cases}D\to Z\\ x \mapsto y(x)\end{cases}}
\end{equation*}

Eine \emph{Umindexierung} wollen wir in dieser Schrift eine \emph{umkehrbare} Abbildung

\begin{equation*}
f \colon \,{
\begin{cases}
D \subseteq \mathbb{R}\to Z \subseteq \mathbb{R}^N \\ 
\xi \mapsto (x_1(\xi),\, x_2(\xi),\, \dots,\, x_N(\xi))
\end{cases}
}
\end{equation*}
nennen, die ein zusammenhängendes Interval in $\mathbb{R}$ auf ein zusammenhängendes Gebiet $\mathcal {G}$ in $\mathbb{R}^N$ abbildet. Eine raumfüllende Kurve kann keine Umindexierung sein, da die Umkehrabbildung $Z \to D$ nicht existiert. Solch eine Umindexierung muss man sich eher wie ein wildes Gehüpfe zwischen Definitions- und Zielmenge vorstellen. Bewegt man sich stetig in der Definitionsmenge, so ist das Bild ein wildes Gehüpfe in der Zielmenge und umgekehrt. Nichtsdestotrotz lassen sich relativ einfach Beispiele für solche Abbildungen formulieren.
%https://math.stackexchange.com/questions/183361/examples-of-bijective-map-from-mathbbr3-rightarrow-mathbbr/183383#183383

Das Kurvenintegral einer stetigen (skalaren) Funktion $\psi$ entlang einer Kurve $\mathcal {K}$ ist definiert als
\begin{equation*}
\int \limits _{\mathcal {K}}\psi\,\mathrm{d} s := 
\int \limits _{a}^{b}\psi\left(x_1(\xi), \dots, x_N(\xi)\right)
\sqrt{ \left(\frac{\mathrm{d} x_1(\xi)}{\mathrm{d}\xi}\right)^2 + \dots + \left(\frac{\mathrm{d} x_N(\xi)}{\mathrm{d}\xi}\right)^2}
\,\mathrm {d} \xi
\end{equation*}
Wenn $\left(x_1(\xi), \dots, x_N(\xi)\right)$ eine Umindexierung ist, dann ist das Kurvenintegral nicht definiert, da die $x_i(\xi)$ im umkehrbaren Fall nicht differenzierbar gewählt werden können. 

Im Fall $N=1$ finden wir 
\begin{equation*}
\int\limits _{a}^{b} \psi(\xi) \,\mathrm {d} \xi =
\int \limits _{a}^{b}\psi\left(x(\xi)\right)
\frac{\mathrm{d} x(\xi)}{\mathrm{d}\xi}
\,\mathrm {d} \xi = 
\int\limits _{x(a)}^{x(b)} \psi(x) \,\mathrm {d} x
\end{equation*}
$\xi$ ist also eine andere Methode, die Punkte auf der x-Achse durchzunummerieren, oder zu \emph{indexieren}.

Nun wollen wir den Ausdruck 
\begin{equation}
\sumint \limits _{a}^{b} \psi(\xi) \,\mathrm {d} \xi 
\end{equation}
als definiert betrachten, wenn es möglich ist, eine Umindexierung $\xi \mapsto (x_1(\xi),\, \dots,\, x_N(\xi))$ zu finden, so dass das Intervall $[a,b]$ auf ein integrierbares Gebiet $\mathcal {G}$ abgebildet wird. Sein Wert soll sein
\begin{equation*}
\int \limits _{\mathcal {G}}\psi(x_1, \dots, x_N) \,\mathrm{d} x_1 \dots \mathrm{d} x_N
\end{equation*}
